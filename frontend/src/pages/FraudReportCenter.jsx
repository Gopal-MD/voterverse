...
export default function FraudReportCenter() {
  ...
  const fetchReports = useCallback(async () => {
    try {
      ...
      const data =
        _data.success !== undefined ? (_data.success ? _data.data : _data) : _data;
  ...
  return (
    <div className="page-container">
      ...
      <div
        ...
        role="tablist"
        aria-label="Fraud center sections"
      >
        <button
      ...
      {tab === 'report' && (
        <div id="panel-report" role="tabpanel" aria-labelledby="tab-report">
          <div className="privacy-notice" role="note">
            ...
            <span>
              Reports are anonymized. Evidence images are <strong>never stored</strong>. Only AI
              classification summaries are saved.
            </span>
          </div>
          ...
          {result ? (
            <div className="glass-card analysis-result" aria-live="polite" role="status">
              ...
              <div className="analysis-row">
                ...
                <span className="analysis-value" style={{ fontFamily: 'monospace' }}>
                  {result.reportId}
                </span>
              </div>
              <div className="analysis-row">
            ...
            <form onSubmit={handleSubmit} className="glass-card">
              <div className="form-group">
                <label htmlFor="fraud-desc" className="form-label">
                  Description *
                </label>
                <textarea
                  id="fraud-desc"
                  ...
                  aria-describedby="fraud-desc-help"
                  maxLength={1000}
                  required
                />
                <small id="fraud-desc-help" style={{ color: 'var(--text-muted)' }}>
                  {form.description.length}/1000 characters
                </small>
              </div>

              <div className="form-group">
                <label htmlFor="fraud-loc" className="form-label">
                  Location *
                </label>
                <input
                  id="fraud-loc"
                  ...
                  placeholder="e.g., Booth #45, Sector 12, Noida"
                  required
                />
              </div>

              <div className="form-group">
                <label htmlFor="fraud-type" className="form-label">
                  Fraud Type
                </label>
                <select
                  id="fraud-type"
              ...
              <div className="form-group">
                <label htmlFor="fraud-evidence" className="form-label">
              ...
              {error && (
                <div
                  role="alert"
                  style={{ color: 'var(--danger)', marginBottom: 16, fontWeight: 600 }}
                >
                  {error}
                </div>
              )}
              ...
              <button
                ...
                disabled={loading || !form.description.trim() || !form.location.trim()}
                id="submit-fraud"
              >
                ...
                <tbody>
                  {reports.map((r) => (
                    <tr key={r.reportId}>
                      ...
                      <td>{r.location}</td>
                      <td>
                        <span className="chip chip-primary">{r.status}</span>
                      </td>
