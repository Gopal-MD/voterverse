    ...
    return (
      <div className="page-container">
        ...
        <div className="glass-card" style={{ textAlign: 'center', padding: 40 }} role="status">
          <div className="quiz-score" aria-label={`Your score: ${score} out of ${total}`}>
            {score}/{total}
          </div>
          <p style={{ color: 'var(--text-muted)', margin: '12px 0', fontSize: '1.1rem' }}>
            {pct >= 80
              ? '🎉 Excellent! You know your elections!'
              : pct >= 50
                ? '👍 Good effort! Keep learning!'
                : '📖 Keep studying — democracy needs informed voters!'}
          </p>
          ...
          <div style={{ marginTop: 24, display: 'flex', gap: 12, justifyContent: 'center' }}>
            <button className="btn btn-primary" onClick={() => startQuiz(topic)}>
              🔄 Retry
            </button>
            <button
  ...
  return (
    <div className="page-container">
      <section className="hero">
        <h1>🧠 Quiz Arena</h1>
        <p>
          Topic: {topic} — Question {total + 1} of 10
        </p>
      </section>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <span className="chip chip-primary" aria-current="step">
          Q{total + 1}/10
        </span>
      </div>

      {loading ? (
        <div className="loading-pulse" aria-live="polite" role="status">
          AI is generating a question for you...
        </div>
      ) : question ? (
        <div className="glass-card">
          <h3 style={{ marginBottom: 20, lineHeight: 1.5 }} id="current-question">
            {question.question}
          </h3>
          ...
          {answered && question.explanation && (
            <div
              style={{
                ...
                borderLeft: `3px solid ${selected === question.correct_index ? 'var(--accent)' : 'var(--danger)'}`,
              }}
              aria-live="polite"
              role="alert"
            >
          ...
          <div style={{ marginTop: 20, display: 'flex', gap: 12 }}>
            {!answered ? (
              ...
              <button
                ...
                id="next-question"
                aria-label={total >= 9 ? 'Finish and see results' : 'Go to next question'}
              >
        ...
        <div className="glass-card" role="alert">
          <p>
            ...
            <button className="btn btn-outline btn-sm" onClick={() => fetchQuestion()}>
              Retry
            </button>
          </p>
        </div>
