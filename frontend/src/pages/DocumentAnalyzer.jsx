...
export default function DocumentAnalyzer() {
  ...
  const handleFile = (f) => {
    if (!f) return;
    if (f.size > 5 * 1024 * 1024) { setError('File size exceeds 5MB limit.'); return; }
    const validTypes = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];
    if (!validTypes.includes(f.type)) { setError('Unsupported file type. Use JPEG, PNG, WebP, or GIF.'); return; }
    setError('');
    setFile(f);
    setAnalysis(null);
  ...
  return (
    <div className="page-container">
      ...
      <div className="privacy-notice" role="note">
        <span>🔒</span>
        <span>Your document is analyzed in-memory only and is <strong>never stored</strong>. The image exists only for the duration of the API call.</span>
      </div>
      ...
      <div className={`upload-zone ${dragOver ? 'drag-over' : ''}`}
        ...
        <input ref={inputRef} type="file" accept="image/*" style={{ display: 'none' }}
          onChange={e => handleFile(e.target.files[0])} id="document-upload" aria-label="Choose document file" />
      </div>

      {preview && (
        <div style={{ margin: '20px 0', textAlign: 'center' }}>
          <img src={preview} alt="Document preview" style={{ maxWidth: '100%', maxHeight: 300, borderRadius: 'var(--radius)', border: '1px solid var(--border)' }} />
        </div>
      )}
      ...
      <div style={{ margin: '20px 0' }}>
        <button className="btn btn-primary" onClick={analyze} disabled={!file || loading} id="analyze-btn">
          {loading ? '⏳ Analyzing...' : '🔍 Analyze Document'}
        </button>
      </div>

      {loading && <div className="loading-pulse" aria-live="polite">AI is analyzing your document...</div>}

      {analysis && (
        <div className="glass-card analysis-result" aria-live="polite">
          <h3 style={{ marginBottom: 16 }}>Analysis Results</h3>
          {fields.map(f => analysis[f.key] ? (
